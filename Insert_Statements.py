...
def insert():
    ...
    room_list = [{'room_number': 30, 'building': DBRef('building', building_results.inserted_id[0])},
                 {'room_number': 23, 'building': DBRef('building', building_results.inserted_id[0])},
                 {'room_number': 402, 'building': DBRef('building', building_results.inserted_id[1])},
                 {'room_number': 105, 'building': DBRef('building', building_results.inserted_id[5])},
                 {'room_number': 213, 'building': DBRef('building', building_results.inserted_id[3])},
                 {'room_number': 309, 'building': DBRef('building', building_results.inserted_id[4])}]
    ...
    key_issue_list = [{'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[0]),
                       'key': DBRef('key', key_result.inserted_id[0])},
                      {'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[1]),
                       'key': DBRef('key', key_result.inserted_id[1])},
                      {'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[2]),
                       'key': DBRef('key', key_result.inserted_id[5])},
                      {'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[3]),
                       'key': DBRef('key', key_result.inserted_id[4])},
                      {'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[4]),
                       'key': DBRef('key', key_result.inserted_id[6])},
                      {'start_time': datetime.now(),
                       'room_request': DBRef('room_request', room_requests_result.inserted_id[5]),
                       'key': DBRef('key', key_result.inserted_id[7])}]

    key_issue_result = key_issue.insert_many(key_issue_list)

    key_issue_return_list = [{'return_date': datetime.now(),
                              'key_issue': DBRef('key_issue', key_issue_result.inserted_id[0])},
                             {'return_date': datetime.now(),
                              'key_issue': DBRef('key_issue', key_issue_result.inserted_id[1])}]

    key_issue_return_result = key_issue_return.insert_many(key_issue_return_list)

    key_issue_loss_list = [{'return_date': datetime.now(),
                            'key_issue': DBRef('key_issue', key_issue_result.inserted_id[2])},
                           {'return_date': datetime.now(),
                            'key_issue': DBRef('key_issue', key_issue_result.inserted_id[3])}]
